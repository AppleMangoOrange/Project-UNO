...
# Implementing the discard pile
'''
The discard pile must only have the 4 top-most cards visible
Rest of the cards go to the top of the deck
Deck is re-shuffled before a player draws from it
'''
discard_pile = []
...
# Main loop
'''
Loops through players' turns and lets them play accordingly
Discard pile rules apply
'''

# Takes a dictionary of list of player hands to see 
# if any have won, and returns the winning player's "name"
def won(hands: dict):
    w = 0
    for i in hands:
        if len(hands[i]) == 0:
